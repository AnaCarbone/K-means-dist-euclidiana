...
public class KMeansDouble {
	...
	private double[][] dados; // matriz de dados que o k-means receberá, cada
	...
	private double max; // valor máximo dos números que podem ser escolhidos
						// aleatoriamente para a criação dos prototipos
	private double min; // valor mínimo dos números que podem ser escolhidos
						// aleatoriamente para a criação dos prototipos
	...
	public KMeansDouble(String arquivo, int k, double taxaErro, int linhas, int colunas) {
		try {
	...
	public double [][] definirPrototipos() {
		...
		for (int i = 0; i < k; i++) {
			...
			for (int j = 0; j < numeroDimensoes; j++) {
				numAleatorio = min + (max - min) * rand.nextDouble();
